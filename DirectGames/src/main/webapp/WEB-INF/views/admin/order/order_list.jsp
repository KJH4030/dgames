        ...
        <table class="table table-sm">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{game_id}}</td>
              <td><img src='/admin/order/imageDisplay?dateFolderName={{img_up_folder}}&fileName={{game_img}}'></td>
              <td>{{game_title}}</td>
              <td>{{ord_price}}</td>
              <td><button type="button"  name="btn_order_delete" class="btn btn-danger" data-ord_code="{{ord_code}}" data-game_id="{{game_id}}">delete</button></td>
            </tr>
            {{/each}}
          </tbody>
        </table>
...
<script>
  ...
  $("table#order_info_tbl").on("click", "button[name='btn_order_delete']", function() {
    ...
    if(!confirm("상품코드 " + game_id + " 번을 삭제하시겠읍니까?")) return;
    ...
    actionForm.attr("action", "/admin/order/order_game_delete");
    actionForm.submit();
